...
软件杯比赛项目，核心产品是 **ZhiPath** — 一个基于多智能体的个性化学习资源生成系统。
...
- **数据库**: PostgreSQL 17 + pgvector（本地安装，端口 5433），embedding: all-mpnet-base-v2
- **不要用 Docker**，本地 PostgreSQL 直接启动（`F:/PostgreSQL/bin/pg_ctl start -D "F:/PostgreSQL/data"`）
- **通信**: WebSocket 流式传输
...
### 启动方式（不用 Docker！本地直接跑）
- 数据库: `F:/PostgreSQL/bin/pg_ctl start -D "F:/PostgreSQL/data"`（端口 5433，密码 123456）
- 后端: `cd LearnFlow/backend && .venv/Scripts/python.exe main.py`（端口 8000）
- 前端: `cd LearnFlow/frontend && npm run dev`（端口 3000）
- DATABASE_URL: `postgresql+asyncpg://postgres:123456@localhost:5433/learnflow`
